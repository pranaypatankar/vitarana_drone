...
class Edrone():
    ...
    def __init__(self):
        ...
        #self.throttle_values = msg.rcThrottle # -----------------------Add other required variables for pid here ----------------------------------------------
        #
        ...
        rospy.Subscriber('/pid_tuning_altitude', PidTune, self.yaw_set_pid)
    ...
    def calc_pid(self):
        ...
        self.roll = self.Kp[0]*self.error[0] + self.error_sum[0]*self.Ki[0] + (self.error[0] - self.prev_values[0] )*self.Kd[0]
        #self.out_roll = self.Kp[0]*self.error[0] +  self.sample_time +  self.Kd[0]*(self.error[0] - self.prev_values[0] )
        self.pitch = self.Kp[1]*self.error[1] + self.error_sum[1]*self.Ki[1] + (self.error[1] - self.prev_values[1] )*self.Kd[1]
        self.yaw = self.Kp[2]*self.error[2] + self.error_sum[2]*self.Ki[2] + self.Kd[2]*(self.error[2] - self.prev_values[2] )
    def prev_error_update(self):
        self.prev_values[0] = self.error[0]
        self.prev_values[1] = self.error[1]
        self.prev_values[2] = self.error[2]

    def assignments(self):
        self.pwm_cmd.prop1 = self.pwm_throttle + self.pitch - self.roll #+ self.yaw
        self.pwm_cmd.prop3 = self.pwm_throttle - self.pitch - self.roll #- self.yaw
        self.pwm_cmd.prop2 = self.pwm_throttle - self.pitch + self.roll #+ self.yaw
        self.pwm_cmd.prop4 = self.pwm_throttle + self.pitch + self.roll #- self.yaw
        #
    ...
    def pid(self):
        ...
        self.pwm_throttle = (self.throttle)*1.023 - 1023
        #
        
        
        
#
        #self.pwm_cmd.prop1 = throttle + self.out_roll + self.out_pitch + self.out_yaw
        #self.pwm_cmd.prop2 = throttle + self.out_roll + self.out_pitch + self.out_yaw
        #self.pwm_cmd.prop3 = throttle + self.out_roll + self.out_pitch + self.out_yaw
        #self.pwm_cmd.prop4 = throttle + self.out_roll + self.out_pitch + self.out_yaw
