...
class Edrone():
	...
	roll_error = self.setpoint_euler[0] - self.drone_orientation_euler[0]
	pitch_error = self.setpoint_euler[1] - self.drone_orientation_euler[1]
	yaw_error = self.setpoint_euler[2] - self.drone_orientation_euler[2]
        # Also convert the range of 1000 to 2000 to 0 to 1024 for throttle here itslef
        #OldRange = (self.max_values - self.min_values)  
        #NewRange = (0 - 1024)  
        #NewValue = (((self.setpoint_cmd[3] - self.min_values) * NewRange) / OldRange) + 0
	# self.sample_time[0] = ( self.sample_time + roll_error) * self.Ki[0]
	#Iterm[1] = (Iterm + pitch_error) * self.Ki[1]
	#Iterm[2] = (Iterm + yaw_error) * self.Ki[2]


	
	self.out_roll = self.Kp[0]*roll_error +  self.sample_time +  self.Kd[0]*(roll_error - self.prev_values[0] )
	self.out_pitch = self.Kp[1]*pitch_error +self.sample_time + self.Kd[1]*(pitch_error - self.prev_values[1] )
	self.out_yaw = self.Kp[2]*yaw_error + self.sample_time +  self.Kd[2]*(yaw_error - self.prev_values[2] )
	...
	self.prev_values[0] = roll_error
	self.prev_values[1] = pitch_error
	self.prev_values[2] = yaw_error
